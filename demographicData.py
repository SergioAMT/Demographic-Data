...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(
        non_percentage_higher / len(higher_education) * 100, 1
    )
    ...
    # country_rich= df[df['salary'] == '>50K']['native-country'].value_counts().idxmax()

    highest_earning_country = (country_rich_count / country_count * 100).idxmax()
    highest_earning_country_percentage = round(
        country_rich_count / country_count * 100, 1
    ).max()

    # Identify the most popular occupation for those who earn >50K in India.
    india = df[df["native-country"] == "India"]
    occupation_count = india["occupation"].value_counts()
